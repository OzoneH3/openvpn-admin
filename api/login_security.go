...
func (l *loginLimiter) allowed(key string) bool {
	...
	a, ok := l.attempts[key]
	if !ok { return true }
	if !a.lockedUntil.IsZero() && now.Before(a.lockedUntil) { return false }
	if now.Sub(a.windowStart) >= loginFailureWindow { delete(l.attempts, key) }
	return true
}
...
func (l *loginLimiter) failure(key string) {
	...
	now := l.now()
	a := l.attempts[key]
	if a.windowStart.IsZero() || now.Sub(a.windowStart) >= loginFailureWindow { a = loginAttempt{windowStart: now} }
	a.failures++
	if a.failures >= loginMaxFailures { a.lockedUntil = now.Add(loginLockout) }
	l.attempts[key] = a
}
...
func loginClientKey(r *http.Request) string {
	...
	if r.RemoteAddr != "" { return r.RemoteAddr }
	return "unknown"
}
